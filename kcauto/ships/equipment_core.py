...
class EquipmentCore(object):
    ...
    def __init__(self):
        ...
        try:
            ...
            self.reinforce_special = JsonData.load_json(
                "data|temp|reinforce_special.json"
            )
            self.ship_type_static = JsonData.load_json("data|temp|ship_type.json")
            self.ship_type_static.insert(
                0, "dummy"
            )  # insert a dummy at index 0 to align with ship type id
            self.equipment_special = JsonData.load_json(
                "data|temp|equipment_ship_special.json"
            )
        except FileNotFoundError as e:
            Log.log_error(
                "Reinforce equipment data not found, please start kcauto from splash screen"
            )
            Log.log_error(e)
    ...
    def goto_fleet(self, fleet_id):
        ...
        while True:
            ...
            if kca_u.kca.exists(
                "upper_left",
                f"fleet|fleet_{fleet_id}_active.png",
                similarity=FLEET_ID_ICON,
            ):
                break
            kca_u.kca.sleep(1)
    ...
    def _is_special_reinforce_equipment(self, ship: Ship, equipment: Equipment):
        ...
        if equipment_str_id in self.reinforce_special:
            if (
                (
                    self.reinforce_special[equipment_str_id].get("api_ship_ids", None)
                    != None
                    and str(ship.api_id)
                    in self.reinforce_special[equipment_str_id]["api_ship_ids"]
                )
                or (
                    self.reinforce_special[equipment_str_id].get("api_ctypes", None)
                    != None
                    and str(ship.ship_class.id)
                    in self.reinforce_special[equipment_str_id]["api_ctypes"]
                )
                or (
                    self.reinforce_special[equipment_str_id].get("api_stypes", None)
                    != None
                    and (
                        str(ship.ship_type.id)
                        in self.reinforce_special[equipment_str_id]["api_stypes"]
                        or str(ShipTypeEnum.WILDCARD.id)
                        in self.reinforce_special[equipment_str_id]["api_stypes"]
                    )
                )
            ):
                if (
                    self.reinforce_special[equipment_str_id]["api_req_level"]
                    > equipment.stars
                ):
                    return False
                else:
    ...
    def get_equipment_by_production_id(
        ...
        for equipment in self.equipment_pool[self.ID]:
            if equipment.production_id == production_id:
                Log.log_warn(
                    f"Cannot find {equipment.name} in specified equipment pool, maybe it is in use"
                )
                return None
    ...
    def is_available_equipments(self, ship: Ship, equipments: list[Equipment]):
        ...
        if target_ship_id in self.equipment_special:
            for i in range(len(local_equipments) - 1, 0 - 1, -1):
                ...
                available_equipments_in_category = self.equipment_special[
                    target_ship_id
                ]["api_equip_type"].get(str(equipment.category), CATEGORY_NOT_AVAILABLE)
                if available_equipments_in_category != CATEGORY_NOT_AVAILABLE:
                    if (
                        available_equipments_in_category == ALL_IS_AVAILABLE
                        or equipment.model_id in available_equipments_in_category
                    ):
                        continue
            ...
            for i in range(len(local_equipments) - 1, 0 - 1, -1):
                equipment = local_equipments[i]
                if (
                    self.ship_type_static[ship.ship_type.id]["api_equip_type"].get(
                        str(equipment.category), 0
                    )
                    == 1
                ):
                    continue
