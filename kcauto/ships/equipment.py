...
class Equipment:
    ...
    def update(
        self, model_id=None, production_id=None, stars=None, lock=None, ace=None
    ):
    ...
    def category_patch(model_id, new_category):
        ...
        if model_id in Equipment.equipment_static_data:
            ...
            Log.log_debug_1(
                f"Patched equipment model_id:{model_id} to category:{new_category}"
            )
        else:
            Log.log_error(
                f"Cannot find model_id:{model_id} in equipment static data, cannot patch category"
            )
