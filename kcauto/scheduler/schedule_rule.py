...
class ScheduleRule(object):
    ...
    def __init__(self, rule_string):
        ...
        if self.condition_type in (
            SchedulerSlot0Enum.TIME,
            SchedulerSlot0Enum.TIME_RUN,
        ):
            self.condition_time = (int(split_rule[1][0:2]), int(split_rule[1][2:4]))
        ...
        if self.action == SchedulerSlot2Enum.SLEEP:
            self.action_length = (int(split_rule[4][0:2]), int(split_rule[4][2:4]))
    ...
    def next_action_time(self):
        ...
        if self.condition_type is SchedulerSlot0Enum.TIME:
            dt = datetime.now().replace(
                hour=self.condition_time[0], minute=self.condition_time[1]
    ...
    def __str__(self):
        ret_string = f"{self.action.display_name} {self.action_module.display_name}"
        if self.action is SchedulerSlot2Enum.SLEEP:
            ret_string += f" for ~{self.action_length[0]:02}:{self.action_length[1]:02}"
        ...
        if self.condition_type in (
            ...
            ret_string += (
                f" after {self.condition_runs} {self.condition_type.display_name}"
            )
        elif self.condition_type is SchedulerSlot0Enum.RESCUE:
