...
def get_current_exp_set(config):
    ...
    for preset in expedition_set:
        if (
            config["expedition.fleet_2"] == expedition_set[preset][0]
            and config["expedition.fleet_3"] == expedition_set[preset][1]
            and config["expedition.fleet_4"] == expedition_set[preset][2]
        ):
            return preset

    if (
        config["expedition.fleet_2"] == ["active"]
        and config["expedition.fleet_3"] == ["active"]
        and config["expedition.fleet_4"] == ["active"]
    ):
        return "active"

    elif (
        config["expedition.fleet_2"] == ["passive"]
        and config["expedition.fleet_3"] == ["passive"]
        and config["expedition.fleet_4"] == ["passive"]
    ):
        return "passive"

    elif (
        config["expedition.fleet_2"] == ["overnight"]
        and config["expedition.fleet_3"] == ["overnight"]
        and config["expedition.fleet_4"] == ["overnight"]
    ):
        return "overnight"
...
def set_config(config, expset, fleet_mode):
    ...
    if fleet_mode == "disable":
        ...
        if expset == "active" or expset == "passive" or expset == "overnight":
