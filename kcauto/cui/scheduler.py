...
def pop_up_menu(stdscr, panel, timestamp, sortie_max_count):
    ...
    if cur_item == "END TIME":
        ...
        while 1:
            ...
            time_str = (
                str((hrs + 24 - 1) % 24).rjust(2, "0")
                + ":"
                + str((mins + 60 - 1) % 60).rjust(2, "0")
            )
            ...
            time_str = (
                str((hrs + 1) % 24).rjust(2, "0")
                + ":"
                + str((mins + 1) % 60).rjust(2, "0")
            )
            panel.addstr(y_center + 1, x_center, time_str, curses.color_pair(LOG))
        ...
        while 1:
            ...
            if sortie_max_count == 0:
                ...
                panel.addstr(
                    y_center + 0,
                    x_center,
                    str(sortie_max_count + 0).rjust(3, "0"),
                    curses.color_pair(LOG_GREEN),
                )
                panel.addstr(
                    y_center + 1,
                    x_center,
                    str(sortie_max_count + 1).rjust(3, "0"),
...
def get_end_time(config):
    for rule in config["scheduler.rules"]:
        if (
            rule.split(":")[0] == "time"
            and rule.split(":")[2] == "stop"
            and rule.split(":")[3] == "kcauto"
        ):
            return rule.split(":")[1]
    return "disable"
...
def get_sortie_count(config):
    for rule in config["scheduler.rules"]:
        if (
            rule.split(":")[0] == "sorties_run"
            and rule.split(":")[2] == "stop"
            and rule.split(":")[3] == "combat"
        ):
...
def set_config(config, end_time, sortie_count):
    for i, rule in enumerate(config["scheduler.rules"]):
        if (
            ...
            config["scheduler.rules"][i] = (
                "sorties_run:" + str(sortie_count) + ":stop:combat"
            )
        elif (
            rule.split(":")[0] == "time"
            and rule.split(":")[2] == "stop"
