...
def pop_up_menu(stdscr, panel, config):
    ...
    while True:
        ...
        panel.addstr(
            row[0],
            col[0],
            ("<" if curser[CURSER_Y] == 0 and current_tab == CONTEXT_SORTIE else " ")
            + "COM"
            + (">" if curser[CURSER_Y] == 0 and current_tab == CONTEXT_SORTIE else " "),
            curses.color_pair(
                SORTIE + (COLOR_REVERT * (int(current_tab == CONTEXT_SORTIE)))
            ),
        )
        panel.addstr(
            row[0],
            col[1],
            ("<" if curser[CURSER_Y] == 0 and current_tab == CONTEXT_PVP else " ")
            + "PVP"
            + (">" if curser[CURSER_Y] == 0 and current_tab == CONTEXT_PVP else " "),
        ...
        panel.addstr(
            ...
            (
                "<"
                if curser[CURSER_Y] == 0 and current_tab == CONTEXT_EXPEDITION
                else " "
            )
            + "EXP"
            + (
            ...
            curses.color_pair(
                EXP + (COLOR_REVERT * (int(current_tab == CONTEXT_EXPEDITION)))
            ),
        )
        panel.addstr(
            row[0],
            col[3],
            ("<" if curser[CURSER_Y] == 0 and current_tab == CONTEXT_FACTORY else " ")
            + "FAC"
            + (
                ">" if curser[CURSER_Y] == 0 and current_tab == CONTEXT_FACTORY else " "
            ),
            curses.color_pair(
                FACTORY + (COLOR_REVERT * (int(current_tab == CONTEXT_FACTORY)))
            ),
        )
        panel.addstr(
            row[0],
            col[4],
            ("<" if curser[CURSER_Y] == 0 and current_tab == CONTEXT_REPAIR else " ")
            + "SUP"
            + (">" if curser[CURSER_Y] == 0 and current_tab == CONTEXT_REPAIR else " "),
            curses.color_pair(
                REPAIR + (COLOR_REVERT * (int(current_tab == CONTEXT_REPAIR)))
            ),
        )
            ...
            for j, quest in enumerate(QUEST_LIST[current_tab][quest_type]):
                ...
                if curser[CURSER_Y] - 1 == j and curser[CURSER_X] == i:
                    ...
                    panel.addstr(
                        row[2],
                        1,
                        quest_info[quest_id]["desc"][0 : width - 2],
                    ...
                    if len(quest_info[quest_id]["desc"]) > width - 2:
                        panel.addstr(
                            row[2] + 1,
                            1,
                            quest_info[quest_id]["desc"][width - 2 : width * 2 - 4],
                            curses.color_pair(LOG),
                        )
                    if (
                        len(quest_info[quest_id]["desc"]) > (width - 2) * 2
                        and row[2] + 2 < height
                    ):
                        panel.addstr(
                            row[2] + 2,
                            1,
                            quest_info[quest_id]["desc"][
                                (width - 2) * 2 : (width - 2) * 3
                            ],
                            curses.color_pair(LOG),
                        )
                ...
                panel.addstr(
                    row[1] + j + y_offset,
                    col[i],
                    (" " if QUEST_LIST[current_tab][quest_type][quest][0] == 0 else "*")
                    + quest.rjust(COL_NEEDED_WIDTH - 1, " "),
                    color,
                )
        ...
        if key == curses.KEY_DOWN or key == ord("j"):
            if curser[CURSER_Y] < len(
                QUEST_LIST[current_tab][QUEST_ORDER[curser[CURSER_X]]]
            ):
                curser[CURSER_Y] += 1
            ...
            if curser[CURSER_Y] == 0:
                ...
                if QUEST_ORDER[curser[CURSER_X]] != YEARLY:
                    if (
                        len(QUEST_LIST[current_tab][QUEST_ORDER[curser[CURSER_X] + 1]])
                        == 0
                    ):
                        continue
                    if curser[CURSER_Y] > len(
                        QUEST_LIST[current_tab][QUEST_ORDER[curser[CURSER_X] + 1]]
                    ):
                        curser[CURSER_Y] = len(
                            QUEST_LIST[current_tab][QUEST_ORDER[curser[CURSER_X] + 1]]
                        )
            ...
            if curser[CURSER_Y] != 0:
                quest = list(
                    QUEST_LIST[current_tab][QUEST_ORDER[curser[CURSER_X]]].keys()
                )[curser[CURSER_Y] - 1]
                if (
