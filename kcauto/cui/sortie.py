...
preset_list = {
    DISABLE: ["disable", "akashi mode"],
    NORMAL: ["disable", "auto", "1", "2", "3"],
}
...
def pop_up_menu(stdscr, panel, cur_mode, sortie_map):
    ...
    if cur_mode == DISABLE:
        ...
        while 1:
            ...
            sortie_map = (
                world_list[world_id - 1].rjust(2, " ")
                + " "
                + map_list[map_id - 1].ljust(2, " ")
            )
            ...
            if map_or_world == WORLD:
                panel.addstr(
                    y_center + 0,
                    x_center,
                    world_list[world_id + 0].rjust(2, " "),
                    curses.color_pair(LOG_GREEN_ACTIVE),
                )
                panel.addstr(
                    y_center + 0, x_center + 2, "-", curses.color_pair(LOG_GREEN)
                )
                panel.addstr(
                    y_center + 0,
                    x_center + 3,
                    map_list[map_id + 0].ljust(2, " "),
        ...
        variant_list = []
        variant_list = Noro6().get_variant(
            "B-" + world_list[world_id] + "-" + map_list[map_id]
        )
        ...
        if is_variant_exist == True:
            ...
            while 1:
                ...
                panel.addstr(
                    y_center + 0, x_center, "    ", curses.color_pair(LOG_GREEN_ACTIVE)
                )
                panel.addstr(y_center + 1, x_center, "    ", curses.color_pair(LOG))
            ...
            sortie_map = (
                world_list[world_id] + "-" + map_list[map_id] + map_variant[variant_id]
            )
        else:
        ...
        if cur_mode == NORMAL:
            while 1:
                for i, preset in enumerate(preset_list[NORMAL]):
                    ...
                    x_center, y_center = util.get_center_str_location(panel, preset)
                    panel.addstr(
                        y_center + i - len(preset_list[NORMAL]) // 2,
                        x_center,
                        preset,
                        color,
                    )
