...
from config.macro import (
    RECIPE_PRESET_CONSTRUCT,
    RECIPE_PRESET_DEVELOP,
    RECIPE_PRESET_CONSTRUCT_TEMPLATE,
    RECIPE_PRESET_DEVELOP_TEMPLATE,
)
...
RESOURCE_PANEL_LAYOUT = [[FUEL_MENU, STEEL_MENU], [AMMO_MENU, BAUXITE_MENU]]
...
def pop_up_menu(stdscr, panel, config):
    ...
    if recipe_preset[CONSTRUCT_TAB] == []:
        try:
            ...
            JsonData.dump_json(
                JsonData.load_json(RECIPE_PRESET_CONSTRUCT_TEMPLATE),
                RECIPE_PRESET_CONSTRUCT,
            )
            recipe_preset[CONSTRUCT_TAB] = JsonData.load_json(RECIPE_PRESET_CONSTRUCT)
    ...
    if recipe_preset[DEVELOP_TAB] == []:
        try:
            ...
            JsonData.dump_json(
                JsonData.load_json(RECIPE_PRESET_DEVELOP_TEMPLATE),
                RECIPE_PRESET_DEVELOP,
            )
            recipe_preset[DEVELOP_TAB] = JsonData.load_json(RECIPE_PRESET_DEVELOP)
    ...
    secretary[CONSTRUCT_TAB], secretary_id[CONSTRUCT_TAB] = _load_secretary(
        config["factory.build_secretary"]
    )
    secretary[DEVELOP_TAB], secretary_id[DEVELOP_TAB] = _load_secretary(
        config["factory.develop_secretary"]
    )
    ...
    tab_col = [
        width // 2 // 2 - len(" construct ") // 2,
        width // 2 + width // 2 // 2 - len(" develop ") // 2,
    ]
    ...
    secretary_col = [
        ...
        x_secretary
        + len("Secretary ship "),  # mode label start  (width: _SEC_MODE_FIELD)
        x_secretary
        + len("Secretary ship ")
        + _SEC_MODE_FIELD,  # ship-id digits start (width: 7)
    ]

    x_recipe, y_recipe = util.get_center_str_location(
        panel, "PRESETNAME  XAMMOX XXXX  XBAUXITEX XXXX"
    )
    recipe_col = [
        x_recipe,
    ...
    while True:
        ...
        panel.addstr(
            TOP_TAB_ROW,
            tab_col[0],
            ("<" if curser[CURSER_Y] == 0 and current_tab == CONSTRUCT_TAB else " ")
            + "Construct"
            + (">" if curser[CURSER_Y] == 0 and current_tab == CONSTRUCT_TAB else " "),
            curses.color_pair(
                CONSTRUCT + (COLOR_REVERT * (int(current_tab == CONSTRUCT_TAB)))
            ),
        )
        panel.addstr(
            TOP_TAB_ROW,
            tab_col[1],
            ("<" if curser[CURSER_Y] == 0 and current_tab == DEVELOP_TAB else " ")
            + "Develop"
            + (">" if curser[CURSER_Y] == 0 and current_tab == DEVELOP_TAB else " "),
            curses.color_pair(
                DEVELOP + (COLOR_REVERT * (int(current_tab == DEVELOP_TAB)))
            ),
        )

        panel.addstr(
            SECRETARY_ROW, secretary_col[0], "Secretary ship ", curses.color_pair(LOG)
        )
        ...
        mode_focused = (
            current_active == TOP_MENU
            and curser[CURSER_Y] == 1
            and (sec_mode != "ID" or curser[CURSER_X] < 2)
        )  # focused if on secretary row and either not in ID mode or not on the digit column
        ...
        if sec_mode == "ID":
            if current_active == SECRETARY_MENU:
                for i in range(7):
                    if i == curser[CURSER_X]:
                        panel.addstr(
                            SECRETARY_ROW - 1,
                            secretary_col[2] + i,
                            str((secretary_id[current_tab][i] + 9) % 10),
                            curses.color_pair(LOG),
                        )
                        panel.addstr(
                            SECRETARY_ROW + 0,
                            secretary_col[2] + i,
                            str(secretary_id[current_tab][i]),
                            curses.color_pair(LOG_GREEN),
                        )
                        panel.addstr(
                            SECRETARY_ROW + 1,
                            secretary_col[2] + i,
                            str((secretary_id[current_tab][i] + 1) % 10),
                            curses.color_pair(LOG),
                        )
                    else:
                        panel.addstr(
                ...
                id_focused = (
                    current_active == TOP_MENU
                    and curser[CURSER_Y] == 1
                    and curser[CURSER_X] == 2
                )
                id_color = curses.color_pair(LOG_GREEN if id_focused else LOG)
                panel.addstr(
                    SECRETARY_ROW,
                    secretary_col[2],
                    str(list_to_int(secretary_id[current_tab])).rjust(7),
                    id_color,
                )
        ...
        for resource_idx, resource in enumerate(RESOURCE_ORDER):
            ...
            if resource == FUEL_MENU:
                ...
                if curser[CURSER_Y] == 3 and curser[CURSER_X] == 2:
                    focus = BAUXITE_MENU
            panel.addstr(
                RESOURCE_ROW_START + row_local_offset,
                recipe_col[col_offset],
            ...
            if current_active == resource:
                for i in range(4):
                    if i == curser[CURSER_X]:
                        panel.addstr(
                            RESOURCE_ROW_START + row_local_offset - 1,
                            recipe_col[col_offset + 1] + i,
                            str((recipe[current_tab][resource_idx][i] + 9) % 10),
                            curses.color_pair(LOG),
                        )
                        panel.addstr(
                            RESOURCE_ROW_START + row_local_offset + 0,
                            recipe_col[col_offset + 1] + i,
                            str(recipe[current_tab][resource_idx][i]),
                            curses.color_pair(LOG_GREEN),
                        )
                        panel.addstr(
                            RESOURCE_ROW_START + row_local_offset + 1,
                            recipe_col[col_offset + 1] + i,
                            str((recipe[current_tab][resource_idx][i] + 1) % 10),
                            curses.color_pair(LOG),
                        )
                    else:
                        panel.addstr(
                ...
                if focus == resource:
                    panel.addstr(
                        RESOURCE_ROW_START + row_local_offset,
                        recipe_col[col_offset + 1],
                        str(list_to_int(recipe[current_tab][resource_idx])).rjust(
                            4, " "
                        ),
                        curses.color_pair(LOG_GREEN),
                    )
        ...
        if (
            current_active == TOP_MENU
            and curser[CURSER_X] == 0
            and curser[CURSER_Y] >= 2
        ):
        ...
        if key == curses.KEY_DOWN or key == ord("j"):
            if current_active == TOP_MENU:
                ...
                secretary[current_tab] = SECRETARY_TYPE_OPTIONS[
                    (idx + 1) % len(SECRETARY_TYPE_OPTIONS)
                ]
            elif current_active == SECRETARY_MENU:
                secretary_id[current_tab][curser[CURSER_X]] = (
                    secretary_id[current_tab][curser[CURSER_X]] + 1
                ) % 10
            elif (
                current_active == FUEL_MENU
                or current_active == AMMO_MENU
                or current_active == STEEL_MENU
                or current_active == BAUXITE_MENU
            ):
                recipe[current_tab][RESOURCE_ORDER.index(current_active)][
                    curser[CURSER_X]
                ] = (
                    recipe[current_tab][RESOURCE_ORDER.index(current_active)][
                        curser[CURSER_X]
            ...
            if current_active == TOP_MENU:
                ...
                secretary_id[current_tab][curser[CURSER_X]] = (
                    secretary_id[current_tab][curser[CURSER_X]] + 9
                ) % 10
            elif (
                current_active == FUEL_MENU
                or current_active == AMMO_MENU
            ...
            if current_active == TOP_MENU:
                if curser[CURSER_Y] == 1:
                    ...
                    if curser[CURSER_X] == 0:
                        ...
                        secretary_int = list(recipe_preset[current_tab].items())[
                            preset_idx
                        ][1][secretary_name]
                        secretary[current_tab], secretary_id[current_tab] = (
                            _load_secretary(secretary_int)
                        )
                        comment[current_tab] = list(recipe_preset[current_tab].items())[
                            preset_idx
                        ][1].get("comment", "")

                        recipe[current_tab] = [
                            int_to_list(n, 4)
                            for n in list(recipe_preset[current_tab].items())[
                                preset_idx
                            ][1][recipe_name]
                        ]

                    else:
                        current_active = RESOURCE_ORDER[
                            (curser[CURSER_Y] - 2) + (curser[CURSER_X] - 1) * 2
                        ]
