...
def resize_panel():
    ...
    next_top = curses.LINES
    next_left = curses.COLS // 4
    log_panel = curses.newwin(
        curses.LINES, curses.COLS - (curses.COLS // 4), 0, curses.COLS // 4
    )
    ...
    for panel in panels:
        if panel == EXP:
            ...
            if config["combat.enabled"] == False:
                ...
                if (
                    config["ship_switcher.enabled"] == True
                    and config["ship_switcher.slots"]
                    == ship_switch.AKASHI_SHIP_SWITCHER_SLOTS
                ):
                    sortie_fleet = ["akashi"]
            else:
...
def open_pop_up(thread, stdscr, active_panel):
    ...
    height = min(
        max(3 * curses.LINES // 5, util.WINDOW_MIN_HEIGHT), util.POP_UP_MAX_HEIGHT
    )
    ...
    if active_panel == EXP:
        ...
        sortie_map, preset, akashi_mode = sortie.pop_up_menu(
            stdscr, popup_win, cur_sortie_mode, sortie_map
        )
        ...
        end_time = scheduler.get_end_time(config)
        sortie_count = scheduler.get_sortie_count(config)
        end_time, sortie_count = scheduler.pop_up_menu(
            stdscr, popup_win, end_time, sortie_count
        )
