...
class ConfigExpedition(ConfigBase):
    ...
    def __init__(self, config):
        ...
        all_expeditions = (
            config["expedition.fleet_2"]
            + config["expedition.fleet_3"]
            + config["expedition.fleet_4"]
        )
        if JsonData.has_str(all_expeditions) == False and len(all_expeditions) != len(
            set(all_expeditions)
        ):
    ...
    def enabled(self, value):
        if type(value) is not bool:
            raise ValueError("Specified value for expedition enabled is not a boolean.")
        self._enabled = value
    ...
    def fleet_3(self, value):
        ...
        if (
            ...
            and len(value) > 0
            and self._config["combat.enabled"]
            and FleetModeEnum.STRIKE.value == self._config["combat.fleet_mode"]
        ):
            raise ValueError(
    ...
    def fleet_preset(self, value):
        if not value:
            ...
            if value != "auto":
                raise ValueError(
                    "The only supported expedition preset is 'auto'/null at the moment."
                )
    ...
    def desire_ammo(self, value):
        if type(value) is not int:
            ...
            raise ValueError(
                "Specified value for desire_ammo is out of range (0-350000)."
            )
    ...
    def desire_bauxite(self, value):
        if type(value) is not int:
            ...
            raise ValueError(
                "Specified value for desire_bauxite is out of range (0-350000)."
            )
    ...
    def desire_steel(self, value):
        if type(value) is not int:
            ...
            raise ValueError(
                "Specified value for desire_steel is out of range (0-350000)."
            )
