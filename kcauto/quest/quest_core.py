...
class QuestCore(CoreBase):
    ...
    def _load_quest_priority(self):
        ...
        quest_priority = JsonData.load_json(
            "data|quests|expedition_quest_priority.json"
        )
        for quest_type in quest_priority:
            for quest_name in quest_priority[quest_type]:
                self.quest_priority_library[CONTEXT_EXPEDITION].append(
                    Quest(quest_name)
                )
    ...
    def update_quest_data(self, data):
        ...
        for raw_quest_api in data["api_list"]:
            if raw_quest_api == -1:
                continue
            self.current_quest_list.append(Quest(api_data=raw_quest_api))
    ...
    def manage_quests(self, context=None, fast_check=True):
        ...
        Log.log_msg(f"Manage {self.CONTEXT_LOOKUP[context]} quests: ")
        ...
        if fast_check == False or is_any_quest_turned_in == True:
            if (
                context != CONTEXT_AUTO_SORTIE
                and context != CONTEXT_AUTO_EXPEDITION
                and context != CONTEXT_AUTO_PVP
            ):
    ...
    def _to_tab(self, tab_name):
        ...
        if (
            kca_u.kca.click_existing(
                "left", f"quest|filter_tab_{tab_name}.png", similarity=NEAR_EXACT
            )
            == True
        ):
            kca_u.kca.wait(
    ...
    def _turn_in_quests(self, context):
        ...
        if (
            kca_u.kca.click_existing(
                "left", "quest|filter_tab_active.png", similarity=NEAR_EXACT
            )
            == True
        ):
            kca_u.kca.wait(
                "left", "quest|filter_tab_active_active.png", similarity=NEAR_EXACT
            )
            api.api.update_from_api({KCSAPIEnum.QUEST_LIST})  # update quest_list

        kca_u.kca.click_existing(
            "lower", "global|page_last.png", pad=PAGE_NAV
        ...
        while i >= 0:
            ...
            if not (quest.name in cfg.config.quest.quests):
                Log.log_debug_1(
                    f"Quest {quest.quest_id}/{quest.name} is not in config."
                )
                if quest.state == QuestStateEnum.DONE:
                    self._turn_in_quest_idx(i)
                    quest_turned_in = True
            elif (
                quest.state == QuestStateEnum.IN_PROGRESS
                and context != CONTEXT_AUTO_SORTIE
                and context != CONTEXT_AUTO_EXPEDITION
                and context != CONTEXT_AUTO_PVP
            ):
                Log.log_msg(
                    f"Checking if quest {quest.name} is relevant to {self.CONTEXT_LOOKUP[context]}."
                )

                deactivate_needed = False
                if not self._is_relevent_quest(quest, context=context):
                    deactivate_needed = True
                    if context != CONTEXT_EXPEDITION and self._is_relevent_quest(
                        quest, context=CONTEXT_EXPEDITION
                    ):
                        deactivate_needed = False
    ...
    def _get_quests_rank_list(self, mode) -> list[Quest]:
        ...
        for quest in self.quest_priority_library[mode]:
            ...
            for current_quest in self.current_quest_list:
                if (
                    current_quest.quest_id != quest.quest_id
                    or current_quest.state == QuestStateEnum.DONE
                ):
                    continue
    ...
    def _toggle_quests(self, context):
        ...
        if (
            kca_u.kca.click_existing(
                "left", "quest|filter_tab_all.png", similarity=NEAR_EXACT
            )
            == True
        ):
            kca_u.kca.wait(
                "left", "quest|filter_tab_all_active.png", similarity=NEAR_EXACT
            )
            api.api.update_from_api({KCSAPIEnum.QUEST_LIST})  # update quest_list
    ...
    def _auto_target_select(self, mode):
        ...
        if (
            kca_u.kca.click_existing(
                "left", f"quest|filter_tab_all.png", similarity=NEAR_EXACT
            )
            == True
        ):
            kca_u.kca.wait("left", f"quest|filter_tab_all_active.png", NEAR_EXACT)
            api.api.update_from_api({KCSAPIEnum.QUEST_LIST})  # update quest_list
            Log.log_msg(
                f"API update done in #{self.CONTEXT_LOOKUP[mode]} auto map select."
            )

        if (
            mode == CONTEXT_AUTO_SORTIE
            and cfg.config.combat.sortie_map_read_only == MapEnum.auto_map_select
        ):
            ...
            if next_quest != []:
                ...
                Log.log_error(
                    f"No sortie quests available, cannot auto select sortie map."
                )
            ...
            if sortie_dict == None:
                Log.log_warn(
                    f"Cannot get quest progress from kc3, use default in config file."
                )
                sortie_list = list(
                    MapEnum(map_name) for map_name in next_quest.recommended_map
                )
                ...
                for map_enum in sortie_dict:
                    for i in range(0, sortie_dict[map_enum]):
                        generated_map_name = (
                            f"{next_quest.name}-{map_enum.world_and_map_and_node}"
                        )
            ...
            if next_quest != []:
                ...
                Log.log_success(
                    f"Attempting to finish PvP quest {pvp.pvp.next_pvp_quest.name}."
                )
            else:
                pvp.pvp.next_pvp_quest = Quest(
                    quest_id=303
                )  # mork the next quest as Cd1, so kcauto use default PvP preset
            ...
            for next_quest in reversed(quest_list):
                Log.log_success(
                    f"Attempting to finish expedition quest {next_quest.name}."
                )
                """Read quest progress"""
                ...
                if exp_dict == None:
                    ...
                    if exp_list == []:
                        Log.log_debug_1(
                            f"Cannot get quest info from kc3 and default config file; kcauto_custom failed to select corresponding expedition."
                        )
                    else:
    ...
    def _click_quest_idx(self, idx):
        ...
        Log.log_msg(
            f"Moving from page {self.cur_page}/{self.tot_page} to {math.ceil((idx + 1) / 5)}/{self.tot_page}."
        )
        ...
        nav.navigate_list.to_page(
            ...
            self.cur_page,
            math.ceil((idx + 1) / 5),
            nav.navigate_list.OP_MODE_QUEST,
        )
    ...
    def _reset_next_quest_reset_time(self):
        ...
        if jst_time.hour == 5:
            temp_time = jst_time.replace(minute=randint(1, 5)) + timedelta(days=1)
        else:
    ...
    def _is_relevent_quest(self, quest: Quest, context):
        ...
        if quest.category.is_repair():
            Log.log_debug_1(
                f"Quest {quest.quest_id}/{quest.name} is a repair/supply quest, which is always enabled."
            )
            return True
        ...
        force_enable = False
        if (
            self.auto_select_quest[context] != None
            and quest.quest_id == self.auto_select_quest[context].quest_id
        ):
            force_enable = True
        ...
        if context == CONTEXT_SORTIE:
            if quest.category.is_sortie() == False:
                Log.log_debug_1(
                    f"Quest {quest.name} {quest.category} is not a combat quest."
                )
                return False
            if len(com.combat.get_sortie_queue()) <= 0:
                ...
                if quest_dict:
                    if not (
                        com.combat.get_sortie_queue()[0].without_quest_and_node_enum
                        in [
                            map_context.without_quest_and_node_enum
                            for map_context in quest_dict.keys()
                        ]
                    ):
                        Log.log_debug_1(
                            f"Quest {quest.name} is not relevant to sortie map {com.combat.get_sortie_queue()[0].without_quest} anymore."
                        )
                        return False
                elif not (
                    com.combat.get_sortie_queue()[0].without_quest_and_node_enum
                    in [
                        map_context.without_quest_and_node_enum
                        for map_context in quest.map_context
                    ]
                ):
                    Log.log_debug_1(
                        f"Quest {quest.name} is not relevant to sortie map {com.combat.get_sortie_queue()[0].without_quest}."
                    )
                    return False
            elif quest.enemy_context != () and not (
                set(com.combat.map_data.enemy_context) & set(quest.enemy_context)
            ):
                Log.log_debug_1(
                    f"Quest {quest.name} is not relevant to enemy context {com.combat.map_data.enemy_context}."
                )
                return False
            elif quest.fleet_composition != {} and not self._is_fleet_valid_for_quest(
                quest, flt.fleets.combat_fleets[0]
            ):
                ...
                if force_enable == True:
                    ...
                    Log.log_warn(
                        f"Quest {quest.name} is the current target quest for {self.CONTEXT_LOOKUP[context]}, force enabling it."
                    )
                else:
                    return False
            ...
            if quest.category.is_expedition() == False:
                Log.log_debug_1(
                    f"Quest {quest.name} {quest.category} is not an expedition quest."
                )
                return False
            ...
            if quest.exp_context != ():
                if quest_dict:
                    if not any(
                        expedition in exp.expedition.cur_exp
                        for expedition in quest_dict.keys()
                    ):
                        Log.log_debug_1(
                            f"Quest {quest.name} is not relevant to expedition {exp.expedition.cur_exp} anymore."
                        )
                        return False
                elif not any(
                    item in exp.expedition.cur_exp for item in quest.exp_context
                ):
                    Log.log_debug_1(
                        f"Quest {quest.name} is not relevant to expedition {exp.expedition.cur_exp}."
                    )
                    return False
            ...
            if quest.category.is_pvp() == False:
                Log.log_debug_1(
                    f"Quest {quest.name} {quest.category} is not a PVP quest."
                )
                return False
            elif quest.fleet_composition != {} and not self._is_fleet_valid_for_quest(
                quest, flt.fleets.pvp_fleets[0]
            ):
                Log.log_debug_1(f"Fleet layout is not valid for quest {quest.name}.")
                if force_enable == True:
                    Log.log_error(f"Fleet layout is not valid for quest {quest.name}.")
            ...
            if quest.category.is_factory() == False:
                Log.log_debug_1(
                    f"Quest {quest.name} {quest.category} is not a factory quest."
                )
                return False
            else:
                return True
        ...
        for requirement_list in quest.fleet_composition:
            for requirement in quest.fleet_composition[requirement_list]:
                ...
                if self.ID in requirement:
                    ...
                    count = fleet.has_stype(
                        [
                            ShipTypeEnum(ship_type)
                            for ship_type in requirement[self.STYPE]
                        ],
                        position,
                    )
                elif self.CTYPE in requirement:
                    count = fleet.has_ctype(
                        [
                            ShipClassEnum(ship_class)
                            for ship_class in requirement[self.CTYPE]
                        ],
                        position,
                    )
                    ...
                    if (
                        fleet.has_stype(
                            requirement[self.ALLOWED_SHIP_TYPES], [0, 1, 2, 3, 4, 5, 6]
                        )
                        != fleet.size
                    ):
                        return False
