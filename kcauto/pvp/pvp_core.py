...
class PvPCore(CoreBase):
    ...
    def _conduct_pvp(self, index):
        pvp_list_region = Region(
            kca_u.kca.game_x + 280, kca_u.kca.game_y + 268 + (index * 82), 795, 64
        )
    ...
    def _get_formation_and_nb(self, api_result):
        ...
        for ship in api_result:
            ...
            data = shp.ships.create_ship(
                shp.ships.get_ship_static_data(api_sortno=0, api_id=ship["api_ship_id"])
            )
    ...
    def _reset_next_pvp_time(self):
        ...
        self.next_pvp_time = KCTime.convert_from_jst(temp_time)
        Log.log_debug_1(f"Next PvP at {KCTime.datetime_to_str(self.next_pvp_time)}.")
