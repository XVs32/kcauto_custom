    ...
    while True:
        ...
        if event == "run_chrome":
            if values["gui.chrome_path"] and values["general.chrome_dev_port"]:
                chrome_cmd_list = [
                    ...
                    "--remote-debugging-port=" + str(values["general.chrome_dev_port"]),
                ]
        ...
        if event == "run_kcauto":
            ...
            proc_reader = threading.Thread(
                target=RuntimeLogger.output_reader, args=(kcauto_proc, gui_queue)
            )
            proc_reader.start()
            ...
            window.TKroot.bind(
                "<Escape>",
                lambda e, w=window, k=kcauto_proc, p=proc_reader: esc_bind(e, w, k, p),
            )
        ...
        if event == "stop_kcauto" or kcauto_proc_poll is not None:
            ...
            window["stop_kcauto"].Update(
                disabled=True, button_color=LayoutBase.COLORS_ALERT_BUTTON_DISABLED
            )
            kcauto_proc_poll = None
            timeout = None
