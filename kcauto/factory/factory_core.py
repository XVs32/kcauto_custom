...
class FactoryCore(object):
    ...
    def develop(self, oil, ammo, steel, bauxite, count):
        ...
        while count > 0:
            ...
            retry = 0
            while (
                not kca_u.kca.exists("lower", "factory|develop_menu.png") and retry < 5
            ):
            ...
            if retry == 5:
                Log.log_error(
                    "Cannot open develop menu, probably because the port is full."
                )
        ...
        while count > 0:
            ...
            for i in range(1, 3):
                if kca_u.kca.exists(build_slot_stat[i], "factory|build_finish.png"):
                    ...
                    if retry == 10:
                        Log.log_error(
                            "Cannot receive ship, probably because the port is full."
                        )
                        Log.log_error("Disabling factory module.")
                        self.enabled = False
                        return False
            ...
            for j in range(1, 3):
                if kca_u.kca.exists(build_slot_stat[j], "factory|build_idle.png"):
                    ...
                    retry = 0
                    while (
                        not kca_u.kca.exists("lower", "factory|develop_menu.png")
                        and retry < 5
                    ):
                    ...
                    if (
                        self.is_large_ship_construction(oil, ammo, steel, bauxite)
                        == True
                    ):
                        kca_u.kca.click_existing(
                            "lower", "factory|large_ship_construction.png"
                        )
                        kca_u.kca.click_existing(
                            "lower", "factory|large_ship_construction_agree.png"
                        )
