...
* 在 `debug`（除錯）模式下啟動 POI（Brave 也停止支援 Manifest V2 了，只能 88 KC3 了 QAQ）
...
* 安裝 `poi-plugin-noro6-exporter` 及 `poi-plugin-forwarder` 擴充外掛
    * 請注意，Noro6 Exporter **不會**將配置匯出至原版的 Noro6 網站，  
      而是匯出到 XVs32 改版（Noro6 審 Pull Request 的速度實在太慢了，沒辦法，只能自己架一個，LOL）。
...
* Windows
    ...
    * 或執行 `.\bin/kcauto_custom\kcauto_custom.exe --cfg <你的設定檔名稱>` 來指定自訂設定檔  
      （注意：此處無需輸入副檔名 `.json`）
* Linux
    * 執行 `./kcauto_cui`
