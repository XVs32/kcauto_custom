...
def init():
    ...
    mapData["panel"] = int(
        input(f"Choose the panel this event map is on(1~9), use 1 if you are not sure:")
    )
...
def on_click(x, y, button, pressed):
    ...
    if click_count == 0:
        ...
        print(
            f"Node {nodeList[click_count - 1]}: [{str(x - upper_left_x).rjust(4)}, {str(y - upper_left_y).rjust(4)}]"
        )

        global mapData
        mapData["nodes"][nodeList[click_count - 1]] = {}
        mapData["nodes"][nodeList[click_count - 1]]["coords"] = [
            x - upper_left_x,
            y - upper_left_y,
        ]
